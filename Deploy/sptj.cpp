...
#include<time.h>
const char * example_img_path = "D:\\DroneDetect\\sptj\\1.png";
const int test_example_num = 1000;
int main()

{
	...
	bool result;		
	start = clock();
	cv::VideoCapture capture("D:\\DroneDetect\\sptj\\DJI_0242.MOV");
	long long frames_count = capture.get(CV_CAP_PROP_FRAME_COUNT);
	for (long long i = 0; i < frames_count; i++)

	{
		printf("the frame of this video is %d,%d",i,frames_count);
		capture.read(image);
		//image = cv::imread(example_img_path, 1);
		d.get_target_loc(image, result, target);
		cv::rectangle(
			image,
			CvPoint(target.get_left_border(), target.get_top_border()),
			CvPoint(target.get_right_border(), target.get_bottom_border()),
			cvScalar(0, 0, 255)
		);
		cv::imshow("", image);
		cv::waitKey(1);
	...
	std::cout << "utter total time is :" << total_time << std::endl;
	fps = test_example_num / total_time;
